resource "aws_ecs_service" "bmi-service" {
  name            = "bmi-service"
  cluster         = module.ecs.var.cluster_id #aws_ecs_cluster.central-cluster.id
  task_definition = module.ecs.var.task_definition_arn #aws_ecs_task_definition.bmi-service.arn
  desired_count   = 1
